...
cats = {'misc' 'tech', 'jmp', 'eye'}; % will be used for output structure and buttons
...
if isfield(cfg, 'inputrej'), 
	...
	elseif ~isfield(inputrej, 'cats') % does the input have rejection categories?
		inputrej.cats = cats; 
	elseif any(~strcmp(inputrej.cats, cats)) 
		error('Cannot handle existing rejected components: Categories do not match.')
	end
...
while err == 0 % KEEP GOING UNTIL THERE IS AN ERROR
	...
	while il < subpl, % il is the subplot count
		...
		if mod(i,subpl)==0 || i == 80			% 80 ???
			...
			%
			tc = uicontrol('Units','normalized','Position',[0.86 0.07 0.075 0.035],...
